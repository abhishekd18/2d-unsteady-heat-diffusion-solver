...
//    dummy = dummy.substr(0,dummy.size()-4).append("mprm.00002");
    file.open(dummy.c_str(), ios::in|ios::binary|ios::ate);
    if (file.is_open()==false)
    {
        cout << "Unable to open file : " << dummy << endl;
    ...
    dummy = dummy.substr(0,dummy.size()-4).append("mipe.").append(ostr.str());

//    dummy = dummy.substr(0,dummy.size()-4).append("mipe.00002");
    ...
    mipe = new int[ne]();
    readStream = new char [ne*sizeof(int)];
    file.seekg (0, ios::beg);
    for(int i=0; i<ne; i++)
    {
        file.read (readStream, sizeof(int));
        swapBytes(readStream, 1, sizeof(int));
        mipe[i] = *((int*)readStream) - 1;
    }
    cout << "> File read complete: " << dummy << endl;
    file.close();
...
//   for(int i=0;i<neg;i++)
//    	cout<<meshG->getMipe()[i]<<"\t"<<meshG->getMprm()[i]<<endl;
    // Sort connectivity array
    for(int i=0;i<neg;i++){
	if(meshG->getMipe()[i] == npe){
        	for(int j=0; j<nen; j++){
		    conn_g[k] = meshG->getElem(meshG->getMprm()[i])->getConn(j);
		    //cout<<k<<"\t"<<conn_g[k]<<endl;
		    k++;
		}
	}
    }
    ...
    int* tmp = new int[k]();
    std::memcpy(tmp,conn_g,k*sizeof(int));
    //delete[] conn_g;
    conn_g = tmp;
...
//cout<<"nnl"<<"\t"<<"conn"<<"\t"<<"map_conn_l_g[nnl]"<<"\t"<<"map_conn_g_l[conn]"<<endl;
	...
	conn = conn_g[i];
	if(prev!=conn){
		map_conn_l_g[kk] = conn;
		map_conn_g_l[conn] = kk;
...
//cout<<nel<<endl;
    ...
    oss << npe; 	     //use the string stream just like cout,except the stream prints not to stdout but to a string.
    dir = settings->getWdir();
    dir = dir.append("proc_").append(oss.str());
