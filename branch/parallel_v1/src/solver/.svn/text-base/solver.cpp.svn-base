...
void femSolver::explicitSolver()
{
    ...
    // Instantiate mpiComm class to enable communication between processors
    mpiComm comm;
	...
	///MPI Communication Send/Recv
	comm.communicate(mesh, M, RHS);

	///Loop through all nodes, calculate and set the temperature (Also check if it reached steady state)
	double rate, max_rate = 0.0, max_rate_gl, T_prev, T_curr, T_max = 0, T_max_global;
	for(int node=0;node<nn;node++){
		...
		// Find the maximum of rate of change of temperature among all the nodes
		if(rate>max_rate)	max_rate = rate;

		// Find the maximum temperature among all the nodes		
		if(T_curr>T_max)	T_max = T_curr; 
	}
	 	
	/// Find the maximum temperature among the processors
	MPI::COMM_WORLD.Reduce(&T_max,&T_max_global,1,MPI::DOUBLE,MPI::MAX,0);

	/// Find the maximum rate of change of temperature among the processors
	MPI::COMM_WORLD.Allreduce(&max_rate,&max_rate_gl,1,MPI::DOUBLE,MPI::MAX);

	/// Stop simulation if max rate of change is below a threshold
	if(max_rate_gl<1){
		if(MPI::COMM_WORLD.Get_rank() == 0){
			cout<<">> Solution reached Steady state! "<<endl;
			cout<<"> Maximum temperature in the domain: "<<T_max_global<<" K\ttime = "<<time<<" s\n"<<endl;
		}
		break;
	}
