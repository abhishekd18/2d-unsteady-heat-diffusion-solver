...
void femSolver::calculateElementMatrices(const int e)
{
	...
	for(int i=0;i<3;i++){
		for(int j=0;j<3;j++){
			K[3*i+j] = K[3*i+j] + \
				  k*(dSdX[i]*dSdX[j] + dSdY[i]*dSdY[j])*det_J_abs*mesh->getME(g)->getWeight();
	...
	for(int i=0;i<3;i++){
		for(int j=0;j<3;j++){
			M[3*i+j] = M[3*i+j] + S[i]*S[j]*det_J_abs*mesh->getME(g)->getWeight();
		}
	}
	...
	for(int i=0;i<3;i++)
		if(X[i]>0.8 && Y[i]<0.4)
		F[i] = F[i] + mesh->getME(g)->getS(i)*settings->getSource()*det_J_abs*mesh->getME(g)->getWeight();
...
{ 
	...
	if(FG!=0){
		...
		///already know the value of temperature and we don't solve for it. 
		if(T1 == settings->getInitT())
			B[n1] = BCValue*HTC*L/2.0;
		if(T2 == settings->getInitT())
			B[n2] = BCValue*HTC*L/2.0;			
		...
		K[3*n1+n1] = K[3*n1+n1] - (-HTC)*L/3;
		K[3*n2+n2] = K[3*n2+n2] - (-HTC)*L/3;
		K[3*n1+n2] = K[3*n1+n2] - (-HTC)*L/6;
		K[3*n2+n1] = K[3*n2+n1] - (-HTC)*L/6;
...
void femSolver::explicitSolver()
{
    ...
    double* M = new double [mesh->getNn()]();
    double* RHS = new double [mesh->getNn()]();
