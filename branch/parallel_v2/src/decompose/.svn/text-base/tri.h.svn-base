...
class triNode
{
    ...
    public:
        /// DEFAULT CONSTRUCTOR
        triNode(){x=0.0f; y=0.0f; T=0.0f; BC_type=0; procb=new int[6]();};
...
class triMesh
{
	...
	int* 	conn_g;		   // Global connectivity array
	int* 	map_conn_l_g;	   // Mapping of connectivity from local to global node numbers
	int* 	map_conn_g_l;	   // Mapping of connectivity from global to local node numbers
	int*	mprm;		   // partition file for element list
	int*	newElemN;	   // renumbered positioning of elements
	int*	nel;
	...
	void setNewElemN   (int* value, int size){std::memcpy(newElemN,value,size*sizeof(int));};
	...
	int*		    getMprm()		{return mprm;};
	int*		    getNewElemN()	{return newElemN;};
