...
{
        cout << endl << "===== Post-processing: time = "<<time<<" s =====" << endl;
...
/*! \brief Main visualization function
 *
 * Writes time stamped VTK datasets for visualization.
 *
 */
void postProcessor::vtkVisualization(int ts, double time)
{
	...
	dummy.append("proc_").append(myrank_str.str()).append("/");
        ...
        dummy = dummy.append(settings->getTitle());
        dummy.append(".");
...
/*
	if(MPI::COMM_WORLD.Get_rank()==0){
		...
		pwriter->SetNumberOfPieces(MPI::COMM_WORLD.Get_size());
