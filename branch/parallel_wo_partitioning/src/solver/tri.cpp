...
void triMesh::readMeshFiles(inputSettings* settings)
{
    ...
    int MAXCOUNT = 20;
    readStream = new char [MAXCOUNT*sizeof(int)];
    file.seekg (0, ios::beg);
    int nprocb, nng;
    ...
    for(int i=0; i<nn; i++)
    {
        file.read (readStream, MAXCOUNT*sizeof(int));
	...
	*((int*)readStream-1);
	...
	if(nprocb>0){
	        for(int j=0; j<MAXCOUNT; j++){
        	    procb[MAXCOUNT*ii+j] = *((int*)readStream+j);
		    if(j>nprocb+1) procb[MAXCOUNT*ii+j] = -1;
		}
		ii++;
	}
    }
